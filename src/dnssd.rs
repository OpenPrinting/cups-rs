...
impl Dnssd {
    ...
    pub fn resolve_service(&self, service: &DnssdBrowseEvent) -> Result<DnssdServiceResolver> {
        // One context serves every browser, resolver and query: a context is an Avahi
        // client connection and a thread of its own, and one per resolution exhausts
        // what Avahi will hand out.
        let context = self.clone();
        let (resolve_sender, resolve_receiver) = mpsc::channel();
        let resolver = context.resolve(service, resolve_sender)?;
...
pub struct DnssdServiceResolver {
    /// The shared context everything here was started on, held so it outlives them.
    context: Dnssd,
    ...
    pub fn try_recv(&mut self) -> Result<Option<DnssdResolvedService>> {
        ...
        while let Ok(service) = self.resolve_receiver.try_recv() {
            ...
            if self.queried.as_ref() == Some(&target) && !self.addresses.is_empty() {
                ...
                match self.context.query_addresses(
                    &service.hostname,
                    service.interface_index,
...
unsafe extern "C" fn resolve_callback(
    ...
    if cb_data.is_null()
        || full_name.is_null()
        || hostname.is_null()
        || (num_txt > 0 && txt.is_null())
        || flags & bindings::cups_dnssd_flags_e_CUPS_DNSSD_FLAGS_ERROR != 0
    {
        return;
    }
    let state = unsafe { &*(cb_data.cast::<ResolveState>()) };
    // Grown rather than reserved: `num_txt` comes from the record, and reserving it
    // up front panics on a length no allocation could satisfy.
